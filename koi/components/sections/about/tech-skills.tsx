...
import { getSkillsByType, getSkills, Skill, getCategoryColor } from '@/components/sections/about/skills-data';

interface TechSkillsProps {
  featuredOnly?: boolean; // only show the most used skills (no categories)
}

const TechSkills = ({ featuredOnly = false }: TechSkillsProps) => {
  const skillsByType = useMemo(() => getSkillsByType(), []);
  const skillCategories = Object.keys(skillsByType);
  const featuredSkills = useMemo(() => getSkills(), []);
  ...
  /**
   * Hide category name; centre section title
   * md: category name left, sticky;
   * xl: section title left
   */
  const styles = {
    container: "flex flex-col justify-center gap-4 xl:gap-6 2xl:gap-8",
    sectionTitle: "text-center xl:text-left text-deep-blue0",
    categoryWrapper: "flex flex-col gap-2",
    categoryNameContainer: "md:sticky md:w-40 xl:w-56",
    categoryName: "hidden md:block font-bold cursor-pointer hover:text-white transition text-lg md:text-lg  xl:flex-row whitespace-nowrap",
  ...
  if (featuredOnly) {
    return (
      <div id="category-container" className={styles.container}>
        ...
        <div id="featured-skills-container" className={styles.skillsContainer}>
          {featuredSkills.map((skill: Skill) => {
            const categoryColor = getCategoryColor(skill.type);
            return (
